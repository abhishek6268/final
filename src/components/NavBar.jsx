    ...
    <>
      <div className="w-full h-36 mobile:h-20  py-2 web:px-12  font-paragraph ">
        <div className=" h-full px-20 mobile:px-12 flex flex-row items-center justify-between">
          ...
          <div
            className={`text-center absolute inset-x-0 z-20 w-full px-6 py-4 h-auto top-20 transition-all duration-300 ease-in-out bg-white dark:bg-gray-800 md:mt-0 md:p-0 md:top-0 md:relative md:bg-transparent md:w-auto md:opacity-100 ${
              isOpen
              ...
              <div className=" web:block flex flex-col gap-12">
                <ul
                  className={`flex gap-2 ${
                    isOpen ? "block flex-col w-full " : "hidden"
                  }`}
                >
            ...
            <ul className="flex gap-4">
              {primarylinks.map(({ name, path }) => {
                return (
                  <Link to={path} key={name}>
                    <li
                      className=" px-1  capitalize text-primary bg-secondary rounded "
